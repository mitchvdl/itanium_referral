 ...
 */
class Itanium_Referral_Adminhtml_Referral_OverviewController extends Mage_Adminhtml_Controller_Action
{
    /**
    ...
    protected function _initAction()
    {
        $this->loadLayout()
            ...
            ->_addBreadcrumb(Mage::helper('itanium_referral')->__('Overview Referrals History'),
                Mage::helper('itanium_referral')->__('Overview Referrals History'));
        return $this;
    }
    ...
    {
        $this->_title($this->__('Itanium'))->_title($this->__('Overview Referrals History'));

        $rateId = $this->getRequest()->getParam('rate_id', 0);
        $rate = Mage::getModel('itanium_referral/reward_history');
        if ($rateId) {
            $rate->load($rateId);
        }
    ...
    public function editAction()
    {
        ...
        $this->_title($rate->getRateId() ? sprintf("#%s", $rate->getRateId()) : $this->__('New Referral History'));
    ...
    public function saveAction()
    {
        ...
        if ($data) {
            ...
            try {
                $rate->save();
                $this->_getSession()->addSuccess(Mage::helper('itanium_referral')->__('The History entry has been saved.'));
            } catch (Exception $e) {
                Mage::logException($e);
                $this->_getSession()->addError($this->__('Cannot save History entry.'));
    ...
    public function deleteAction()
    {
        ...
        if ($rate->getId()) {
            try {
                $rate->delete();
                $this->_getSession()->addSuccess(Mage::helper('itanium_referral')->__('The History has been deleted.'));
            } catch (Exception $e) {
