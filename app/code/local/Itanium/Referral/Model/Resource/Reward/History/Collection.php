<?php
/**
 * Referral history collection
 *
 * @category    Itanium
 * @package     Itanium_Referral
 */
class Itanium_Referral_Model_Resource_Reward_History_Collection extends Mage_Core_Model_Resource_Db_Collection_Abstract
{
    /**
     * Expiry config
     *
    ...
    {
        $this->_init('itanium_referral/reward_history');
    }

    /**
     * Join reward table and retrieve total balance total with customer_id
     *
     * @return Itanium_Referral_Model_Resource_Reward_History_Collection
     */
    protected function _joinReward()
    {
        ...
        $this->getSelect()->joinInner(
            array('reward_table' => $this->getTable('itanium_referral/reward')),
    ...
    public function addCustomerInfo()
    {
        ...
        $warningNotification = $customer->getAttribute('referral_warning_notification');
...
//            )
    ...
    public function addExpirationDate($websiteId = null)
    {
        ...
        if ($websiteId !== null) {
            $field = $expiryConfig->getExpiryCalculation()== Itanium_Referral_Model_Source_Points_ExpiryCalculation::CALC_STATIC ? 'expired_at_static' : 'expired_at_dynamic';
            $this->getSelect()->columns(array('expiration_date' => $field));
        } else {
            $cases = array();
            foreach ($expiryConfig as $wId => $config) {
                $field = $config->getExpiryCalculation()== Itanium_Referral_Model_Source_Points_ExpiryCalculation::CALC_STATIC ? 'expired_at_static' : 'expired_at_dynamic';
                $cases[$wId] = $field;
            }
