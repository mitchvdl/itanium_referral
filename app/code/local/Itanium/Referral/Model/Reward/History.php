<?php
/**
 * Referral history model
 *
 * @category    Itanium
 * @package     Itanium_Referral
  */
class Itanium_Referral_Model_Reward_History extends Mage_Core_Model_Abstract
{
    ...
    protected function _beforeSave()
    {
        ...
        $lifetime = (int)Mage::helper('itanium_referral')->getGeneralConfig('expiration_days', $this->getWebsiteId());
        if ($lifetime > 0) {
    ...
    public function getMessage()
    {
        if (!$this->hasData('message')) {
            $action = Mage::getSingleton('itanium_referral/reward')->getActionInstance($this->getAction());
            $message = '';
            if ($action !== null) {
    ...
    public function getRateText()
    {
        ...
        if (isset($rate['points']) && isset($rate['currency_amount']) && isset($rate['direction'])) {
            return Itanium_Referral_Model_Reward_Rate::getRateText(
    ...
    public function getExpiresAt()
    {
        ...
        return Mage::helper('itanium_referral')->getGeneralConfig('expiry_calculation') == 'static'
